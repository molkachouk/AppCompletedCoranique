...
const [teacher,setTeacher] = useState(null);
  ...
  (async()=> {
    try {
      ...
      const result = await axios.get(`http://localhost:5000/api/Teacher/${id}`);
      if(result)
      {
        setLoading(false);
              ...
              <Form>
                <Row className="mb-3">
                  <Col md={6}>
                    ...
                    <Form.Control type="text" value={teacher.name} readOnly />
                
                    <Form.Label className="small mb-1">عنوان البريد الإلكتروني</Form.Label>
                    <Form.Control type="text" value={teacher.email} readOnly />
                  </Col>
